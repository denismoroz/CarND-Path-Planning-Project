...
ppp::Planner::Planner(Vehicle* car, Map* map):
  car(car), map(map), _velocity(0)
{
    state = PlannerStatePtr(new KeepLaneState(this));
}


void ppp::Planner::_generate_path(int lane) {
...
void ppp::Planner::_update_velocity(double change) {
  if (change > 0) {
...
void ppp::Planner::check(const Road& road){
  this->road = road;

  ppp::NextAction a = state->next_action();
